...
describe 'kerberos::realm', type: :define do
  context 'on a Debian OS' do
    ...
    describe 'with the minimum kdc server string' do
      ...
      it {
        is_expected.to contain_concat__fragment('krb5_EXAMPLE.ORG_realm').with(
          'target'  => 'krb5_config',
          'order'   => '04exampleorg',
        )
      }
      it {
        is_expected.to contain_concat__fragment('krb5_EXAMPLE.ORG_realm').with_content(
          %r{^  EXAMPLE.ORG = \{$},
        )
      }
      it {
      ...
      it {
        is_expected.to contain_concat__fragment('krb5_EXAMPLE.ORG_realm').without_content(
          %r{^    master_kdc     = .*$},
        )
      }
      it {
